...
public class Main3D extends Application {
    ...
    final Canvas canv2 = new Canvas(fullScreen ? primaryScreenBounds.getWidth() : windowWidth,
            fullScreen ? primaryScreenBounds.getHeight() : windowHeight);
    final Canvas canv13d = new Canvas(fullScreen ? primaryScreenBounds.getWidth() : windowWidth,
            fullScreen ? primaryScreenBounds.getHeight() : windowHeight);
    final Canvas canv23d = new Canvas(fullScreen ? primaryScreenBounds.getWidth() : windowWidth,
            fullScreen ? primaryScreenBounds.getHeight() : windowHeight);
    ...
    GraphicsContext dlayer2 = canv13d.getGraphicsContext2D();
    GraphicsContext dlayer1 = canv23d.getGraphicsContext2D();
    Pane pane = new Pane();
    ...
    public void start(Stage stage) throws Exception {
        ...
        canv13d.setFocusTraversable(true);
        canv23d.setFocusTraversable(false);

        pane.getChildren().add(canv1);
        pane.getChildren().add(canv2);
        pane3d.getChildren().add(canv13d);
        pane3d.getChildren().add(canv23d);
        canv1.toFront();
        canv13d.toFront();
        borderPane.setCenter(pane);
        borderPane3d.setCenter(pane3d);
        root.getChildren().add(borderPane);
        root.getChildren().add(borderPane3d);
        PerspectiveTransform perspective = new PerspectiveTransform();
        perspective.setUlx(500.0);
        perspective.setUly(300.0);

        perspective.setUrx(866.0);
        perspective.setUry(300.0);

        perspective.setLlx( -50.0);
        perspective.setLly(768.0);

        perspective.setLrx(1416.0);
        perspective.setLry(768.0);

        borderPane3d.setEffect(perspective);
    ...
    void drawNote(int lane, double d, double currentNote, double e, double currentNote2) {
        ...
        case 0:
            dlayer2.setFill(Color.rgb(0, 0, 220));
            dlayer2.fillPolygon(
            ...
            break;
        case 1:
            dlayer2.setFill(Color.rgb(0, 220, 220));
            dlayer2.fillPolygon(
            ...
            break;
        case 2:
            dlayer2.setFill(Color.rgb(220, 0, 220));
            dlayer2.fillPolygon(
                    new double[] {lane * (width / 4), lane * (width / 4) + noteSize, (lane + 1) * (width / 4)
                            - noteSize, (lane + 1)
                                    * (width / 4), (lane + 1) * (width / 4) - noteSize, lane * (width / 4) + noteSize},
            ...
            dlayer2.fillPolygon(
                    ...
                    6);
            rect(dlayer2, lane * (width / 4) + 30, d - (e * currentNote2), width / 4 - 60, e * currentNote2, 200, 0,
                    200,
                    0.5);
            dlayer2.setStroke(Color.rgb(255, 0, 255));
            dlayer2.strokeLine(lane * (width / 4) + (width / 8), d - (e * currentNote2),
                    lane * (width / 4) + (width / 8),
                    d);
            break;
        case 3:
            dlayer2.setFill(Color.rgb(0, 220, 0));
            dlayer2.fillPolygon(
    ...
    void drawScreen(String condition) {
        ...
        if ( !condition.equals("game")) {
            dlayer1.clearRect(0, 0, width, height);
            dlayer2.clearRect(0, 0, width, height);
        }
        ...
        case "game":
            ...
            layer1.setStroke(Color.DARKSLATEGRAY);
            rect(layer1, 0, 0, width, height, 0, 0, 0, 1);
            for (int i = 0; i < 100; i++) {
                layer1.strokeLine(i * 50, 0, i * 50, height);
            }
            dlayer2.setFill(Color.color(1, 1, 1, 0));
            dlayer2.clearRect(0, 0, width, height);
            dlayer1.setLineWidth(3);
            rect(dlayer1, 0, 0, width, height, 0, 0, 0, 0.15);
            dlayer1.setFill(Color.BLACK);
            dlayer1.setStroke(Color.WHITE);
            if (drawJudgeLine) {
                dlayer1.setLineWidth(1);
                dlayer1.strokeLine(0, height - judgeLine, width, height - judgeLine);
            }
            for (int lane = 0; lane < 5; lane++) {
                dlayer1.setLineWidth(3);
                dlayer1.strokeLine((width / 4) * lane, 0, (width / 4) * lane, height);
                dlayer1.strokePolygon(
                        new double[] {lane * (width / 4), lane * (width / 4) + noteSize, (lane + 1) * (width / 4)
                        ...
                        6);
                dlayer1.setLineWidth(1);
                dlayer1.strokeLine((lane * (width / 4)) + (width / 8), 0, (lane * (width / 4)) + (width / 8), height);
            }
            if (auto) {
        ...
        case "menu":
            ...
            //layer2.fillText("E"+difficulty[currentSong][0]+" N"+difficulty[currentSong][1]+" H"+difficulty[currentSong][2]+" L"+difficulty[currentSong][3], 1100*disp, 100*disp);
            layer2.setFont(Font.font("Times New Roman", FontWeight.BOLD, 80 * disp));
            //layer2.fillText(names[currentSong], 570 * disp, 580 * disp);
    ...
    void drawEffect(String kind, int lane) {
        ...
        case "press":
            ...
            dlayer1.setFill(gra3);
            dlayer1.fillRect(lane * (width / 4), 0, width / 4, height);
            break;
        case "attack":
            stops3 = new Stop[] {new Stop(0, Color.color(0, 0, 0, 0)), new Stop(1,
                ...
                if (currentNote != null && currentNote[6] == currentdif) {
                    ...
                    if ( !auto) {//オートモード
                        ...
                        if (signal[lane] == 2 || signal[lane] == 5) {//長押し状態であるか？
                            ...
                            if (currentNote[2] == 3 && frame == currentNote[0]) {
                                judge(currentNote[2], lane, loop, 0);
                            }
    ...
    void hantei(String h, int b, boolean com, int l3, double p, int id, int lane, int lag) {
        dlayer1.setTextAlign(TextAlignment.CENTER);
        double[] local = notemap.get(id);
        hantei = h;
        breakdown[b]++;
        if (com) {
        ...
        switch (h) {
        case "Lost":
            rect(dlayer1, lane * (width / 4), 0, width / 4, height, 255, 0, 0, 0.8);
            break;
        case "Fair":
            rect(dlayer1, lane * (width / 4), 0, width / 4, height, 30, 30, 255, 0.8);
            break;
        case "Good":
            rect(dlayer1, lane * (width / 4), 0, width / 4, height, 0, 200, 0, 0.8);
            break;
        case "Perfect":
            if (lag != 256) rect(dlayer1, lane * (width / 4), 0, width / 4, height, 255, 255, 0, 0.8);
            break;
        }
        System.out.println(h + "    " + id + " " + lag + " " + frame);
        dlayer1.setFont(Font.font("Times New Roman", FontWeight.BOLD, 48));
        dlayer1.setLineWidth(2);
        dlayer1.setStroke(Color.color(0, 0, 0));
        dlayer1.setFill(Color.rgb(255, 255, 255));
        dlayer1.fillText(hantei, lane * (width / 4) + (width / 8), height - 300);
        dlayer1.strokeText(hantei, lane * (width / 4) + (width / 8), height - 300);
        if ( !h.equals("Lost")) {
            //drawEffect("hit", lane);
        }
    }
    ...
    void gameStart(String name, int dif) {
        ...
        scores = new double[] {(((double) limits[dif - 1] / (double) notes) * 10.0), ((double) limits[dif - 1]
                / (double) notes
                * 7.5), (((double) limits[dif - 1] / (double) notes) * 2.5)};
        if (notes == 0) {
    ...
    void rank() {
        if ((double) score / (double) limits[currentdif - 1] > 0.95) {
            clearStatus = "Success";
            clearRank = "SSS";
            rankColor = Color.rgb(255, 255, 50);
        } else if ((double) score / (double) limits[currentdif - 1] > 0.92) {
            clearStatus = "Success";
            clearRank = "SS";
        ...
        } else if ((double) score / (double) limits[currentdif - 1] > 0.8) {
            clearStatus = "Success";
            clearRank = "A+";
            rankColor = Color.rgb(220, 30, 30);
        } else if ((double) score / (double) limits[currentdif - 1] > 0.7) {
            clearStatus = "Success";
            clearRank = "A";
            rankColor = Color.rgb(220, 0, 0);
        } else if ((double) score / (double) limits[currentdif - 1] > 0.6) {
            clearStatus = "Success";
            clearRank = "B";
